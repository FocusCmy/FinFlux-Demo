...
(function () {
  ...
  function runtimeColumn(ws, guard) {
    ...
    var agentResult = active.agent_result || {}, workerArtifacts = agentResult.worker_artifacts || {}, routePlan = ((active.root_route_decision || {}).worker_plan || {});
    var plannedWorkerCount = Number(routePlan.count || (routePlan.workers || []).length || agentResult.workers_required || 3);
    var completedWorkerCount = Object.keys(workerArtifacts).filter(function (role) {
      var artifact = workerArtifacts[role] || {}, digest = String(artifact.artifact_sha256 || artifact.seal_hash || "");
      return Boolean(artifact.task_id) && String(artifact.run_id || "") === String(active.run_id || "") && /^[0-9a-f]{64}$/i.test(digest);
    }).length;
    var managerParticipated = Boolean(active.agentteams_run_id && active.root_route_decision);
    var caseLeadParticipated = Boolean(active.agentteams_run_id && (agentResult.leader_recommendation || agentResult.leader_datapass_event_id));
    var collaborationCompleted = completedWorkerCount + (managerParticipated ? 1 : 0) + (caseLeadParticipated ? 1 : 0);
    var collaborationRequired = plannedWorkerCount + 2;
    var multiAgentSatisfied = plannedWorkerCount >= 3 && completedWorkerCount >= 3 && collaborationCompleted >= 5;
    ...
    return '<section class="runtime-column"><div class="col-title"><span class="col-num">3</span><div><p class="eyebrow">RESILIENT AGENT CONTROL</p><h2>高可用协作控制面</h2></div></div>' +
      ...
      '<div class="card ha-card"><div class="card-head"><i class="ri-heart-pulse-line card-icon"></i><h4>RunSupervisor 后台推进</h4>' + window.FinfluxUI.badge(supervisor.state || "STOPPED") + window.FinfluxUI.badge(multiAgentSatisfied ? "MULTI_AGENT_3_PLUS" : "MULTI_AGENT_RUNNING") + '</div><div class="ha-stats"><span><b>' + esc(completedWorkerCount) + '/' + esc(plannedWorkerCount) + '</b>核心Worker产物</span><span><b>' + esc(collaborationCompleted) + '/' + esc(collaborationRequired) + '</b>协作Agent职责</span><span><b>' + esc(supervisor.interval_seconds || 5) + 's</b>同步间隔</span></div><p class="card-note">' + esc(supervisor.last_action || cp.recommended_action || "等待后台状态") + (supervisor.wait_reason ? ' · ' + esc(supervisor.wait_reason) : '') + '</p><small>运行时角色就绪 ' + esc(cp.topology_ready || 0) + '/' + esc(cp.topology_expected || 0) + '；运维恢复已执行 ' + esc(supervisor.recovery_count || 0) + ' 次（上限3次），它不是Agent数量。后台独立同步Matrix、Worker产物和模型网关账本。</small><button id="btn-reconcile" class="btn btn-outline" type="button"><i class="ri-refresh-line"></i> 仅执行控制面健康检查</button></div>' +
